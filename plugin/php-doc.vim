...
let g:pdv_cfg_Package               = get(g:,
    \ 'pdv_cfg_Package', "")

let g:pdv_cfg_Version               = get(g:,
    \ 'pdv_cfg_Version', "$id$")

let g:pdv_cfg_Author                = get(g:,
    \ 'pdv_cfg_Author', "Tobias Schlitt <toby@php.net>")

let g:pdv_cfg_Copyright             = get(g:,
    \ 'pdv_cfg_Copyright', '1997-2005 The PHP Group')

let g:pdv_cfg_License               = get(g:,
    \ 'pdv_cfg_License', 'PHP Version 3.0 {@link http://www.php.net/license/3_0.txt}')

let g:pdv_cfg_ReturnVal = "void"

" Wether to create @uses tags for implementation of interfaces and inheritance
let g:pdv_cfg_Uses                    = get(g:,
    \ 'pdv_cfg_Uses', 1)
...
let g:pdv_re_func = '^\s*\([a-zA-Z ]*\)function\s\+\([^ (]\+\)\s*(\s*\(.*\)\s*)\(\s*:\s*[a-zA-Z]*\)\?\s*[{;]\?$'
...
func! PhpDocFunc()
  ...
  let l:indent = matchstr(l:name, g:pdv_re_indent)
	...
	let _return = substitute (l:name, g:pdv_re_func, '\4', "g")
  let l:return = substitute (_return, '\s*:\s*\([a-zA-Z]*\)', '\1', "")
  ...
  if l:return != ""
      exe l:txtBOL . g:pdv_cfg_Commentn . "@return " . l:return . g:pdv_cfg_EOL
  else
...
func! PhpDocType(typeString)
    ...
    if a:typeString =~ g:pdv_re_array_simple
        let l:type = "array"
    endif
